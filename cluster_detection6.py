...
class Cluster:
    def __init__(self, reference_position):
        ...
        self.score = None
        self._position = None
        self._min_distance = float('inf')  # Initialize with a large value
    ...
    def add_point(self, point):
        ...
        # Calculate distance from point to reference position
        distance = ((point[0] - self.reference_position[0]) ** 2 + (point[1] - self.reference_position[1]) ** 2) ** 0.5
...
class DBSCAN:
    def __init__(self, eps=1.5, min_samples=0, alpha=1, beta=1):
        ...
        for x in range(self.grid.shape[0]):
            for y in range(self.grid.shape[1]):
                ...
                if any(self.grid[nx, ny] == 0 for nx, ny in neighbors):
                    new_cluster = Cluster(reference_position)
